                    ...
                    <div id="accordion-collapse" data-accordion="collapse" class="mt-4 pe-3">
                        @foreach ($areas as $area)
                            <h2 id="accordion-heading-{{ $area->id }}">
                                <button type="button" class="flex items-center justify-between w-full p-4 font-medium text-left text-gray-800 border border-gray-200 rounded-lg focus:ring-4 focus:ring-maroon-800 hover:text-white focus:text-white hover:bg-maroon-800 focus:bg-maroon-800" data-accordion-target="#accordion-body-{{ $area->id }}" aria-expanded="false" aria-controls="accordion-body-{{ $area->id }}">
                                    <span cclass="text-gray-800">{{ $area->name }}</span>
                            ...
                            <div id="accordion-body-{{ $area->id }}" class="hidden" aria-labelledby="accordion-heading-{{ $area->id }}">
                                <ul class="p-4 space-y-2 border border-gray-200 rounded-lg">
                                    @foreach ($area->parameters()->orderBy('name','asc')->get() as $parameter)
                                        <li>
                                            <a href="{{ route('area.show', ['program_code' => $program_code, 'area' => $area->name, 'parameter' => $parameter->name]) }}" class="text-maroon-700 hover:underline">
                                                {{ $parameter->name }}
                                            </a>
                                        </li>
                                    @endforeach
